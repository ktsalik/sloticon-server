...
try {
  ...
  balance = parseFloat(localState.balance);
} catch {
  ...
  if (reels.active) {
    reels.forEach((r, i) => {
      if ((r.rolling == true || r.stopping < r.positions + 1) && !(r.forceStopped || r.stoppedAutomatically)) {
        const stopValues = [];
        for (let k = 0; k < r.positions + 1; k++) {
          stopValues.push(parseInt(Math.random() * symbolsCount) + 1);
        }
        r._stopValues = stopValues;
        r.values = r._stopValues.slice();
        r.offset = 0;
    ...
    const betValue = Math.trunc((bet * coinValue) * 100) / 100;
    const canBet = balance >= betValue;
...
function init() {
  ...
  for (let i = 0; i < 5; i++) {
    let values = [];
    if (localState) {
      values = localState.reels[i];
    } else {
      for (let k = 0; k < 4; k++) {
        values.push(parseInt(Math.random() * symbolsCount) + 1);
      }
    }
    ...
    for (let i = 0; i < reels.length; i++) {
      ...
      if (active) {
        ...
        if (reel.rollingTime > reelStopTime) {
          const stopValues = [];
          for (let k = 0; k < reel.positions + 1; k++) {
            stopValues.push(parseInt(Math.random() * symbolsCount) + 1);
          }
          reel._stopValues = stopValues;
          reel.stop();
...
function initControls() {
  ...
  const creditsValue = new PIXI.Text(balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), {
    fontFamily: 'Archivo Black',
    fontSize: 20,
    fill: '#FFFFFF',
  });
  ...
  const betValue = new PIXI.Text((bet * coinValue).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }), {
    fontFamily: 'Archivo Black',
    fontSize: 20,
    fill: '#FFFFFF',
  });
  ...
  reels.onStart(() => {
    ...
    btnPlayCircleRotationTween = gsap.to(btnPlayCircleRotation, { value: 0.001, duration: 2.5 });
  });
...
reels.onStop(() => {
  ...
  localState.reels = reels.map((reel) => reel.values);
